...
void ClusterLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const int num_output = this->layer_param_.cluster_loss_param().num_output();  
  N_ = num_output;
  const int axis = bottom[0]->CanonicalAxisIndex(
      this->layer_param_.cluster_loss_param().axis());
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > cluster_filler(GetFiller<Dtype>(
        this->layer_param_.cluster_loss_param().cluster_filler()));
...
void ClusterLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  center_distance_.Reshape(1,N_,N_,K_);
  center_among.Reshape(1,1,N_,K_);
